package punk.transition
{
	...
	public class Transition
	{
		...
		/**
		 * Transition to the same world by covering up the screen
		 * @param	outEffect
		 * @param	inEffect
		 */
		public static function reloadWorld(outEffect:Effect, inEffect:Effect, options:Object = null):void
		{
			_outWorld = FP.world
			...
			// Create out effects
			_out = outEffect;			
			_out.onComplete = onReloadOut;
			_out.active = true; // all effects are inactive by default
			
			// Prepare in effects
			_in = inEffect;
			_in.onComplete = onReloadIn;
			
			_outWorld.add(_out);	
		}
		
		static private function onReloadOut():void 
		{
			//trace("[Transition] onReloadOut()");
			_outWorld.remove(_out);
			_out.active = false;
			if (_onOutComplete != null) _onOutComplete();
			// Turn on in effect
		...
		static private function onReloadIn():void 
		{
			//trace("[Transition] onReloadIn()");
			if (_onInComplete != null) _onInComplete();
			_in.active = false;
			_outWorld.remove(_in);
